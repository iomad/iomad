...
class cm_completion implements renderable, templatable {
     ...
     * @return int
     */
	//THOMAS START 23.12.24
	//original function deactivated
    ...
    final public function get_completion_state(): int { 
        global $DB;
        $design = \local_designer\options::get_options($this->cm->id);

       //if key exists, check if it is true, then apply new procedure
        if (isset($design->activityelements['completionbadgepersection'])) {
            if ($design->activityelements['completionbadgepersection'] == true) {
                $sectionId = $this->cm->section;
                $courseId = $this->cm->course;
                $modinfo = get_fast_modinfo($courseId);
        
                $sql = "SELECT cm.id FROM {course_modules} cm WHERE cm.section = :sectionid AND cm.course = :courseid";
                $params = ['sectionid' => $sectionId, 'courseid' => $courseId];
                $cms = $DB->get_records_sql($sql, $params);
                foreach ($cms as $cmid => $data) {
                    $cmInfo = $modinfo->get_cm($cmid);
                    if (self::check_completion($cmInfo) == 1) {
                        return 1; // Early exit if any module is complete
                    }
                }
                return 0; // Return 0 if no modules are complete  
            } else 
            //if it is not true, apply original procedure
            {
                return !is_null($this->get_completion_data()->completionstate) ? $this->get_completion_data()->completionstate : 0; 
            }  
        } else
        //if it does not exist, apply original procedure
        {
            return !is_null($this->get_completion_data()->completionstate) ? $this->get_completion_data()->completionstate : 0; 
        }   
    }

    // New function to check all activity completions in a section
	final public function get_completion_state_new(): int {
        global $DB;
	...
	//Static method for checking completion state (performance upgrade due to less recursions)
    public static function check_completion(cm_info $cmInfo) {
        $courseId = $cmInfo->course;	
        if (!isset(self::$completioninfos[$courseId])) {
            self::$completioninfos[$courseId] = new completion_info($cmInfo->get_course());
        }
        $completionData = self::$completioninfos[$courseId]->get_data($cmInfo, true);
	...
	final public function get_completion_date(): ?int {
		...
		$sql = "SELECT cm.id FROM {course_modules} cm WHERE cm.section = :sectionid AND cm.course = :courseid AND cm.completion != 0";
		$params = ['sectionid' => $sectionId, 'courseid' => $courseId];
		$cms = $DB->get_records_sql($sql, $params);
		...
		foreach ($cms as $cmid => $data) {
			$cmInfo = $modinfo->get_cm($cmid);
			$completionData = self::check_completion_data($cmInfo);
			if ($completionData->completionstate == 1) {
				if (is_null($latestCompletionDate) || $completionData->timemodified > $latestCompletionDate) {
					$latestCompletionDate = $completionData->timemodified;
				}
			}
		}
	...
	// Static method for checking completion state and retrieving completion data
	public static function check_completion_data(cm_info $cmInfo): stdClass {
		$courseId = $cmInfo->course;
		if (!isset(self::$completioninfos[$courseId])) {
			self::$completioninfos[$courseId] = new completion_info($cmInfo->get_course());
		}
		return self::$completioninfos[$courseId]->get_data($cmInfo, true);
	}
	//END THOMAS
