...
function format_designer_coursemodule_standard_elements($formwrapper, $mform) {
    ...
    if ($course->format == 'designer') {
        ...
        //START THOMAS 24.12.24: Add drop-down to select if completionbadge is per section or not
        $elements2 = [
            'completionbadgepersection' => 0,
        ];
        $choice2 = [
            0 => get_string('activity:completionbadgepersection:no', 'format_designer'),
            1 => get_string('activity:completionbadgepersection:yes', 'format_designer'),
        ];
        foreach ($elements2 as $element => $defalut) {
            // Module background image repeat.
            $name = 'designer_activityelements['.$element.']';
            $title = get_string('activity:'.$element, 'format_designer');
            $mform->addElement('select', $name, $title, $choice2);
            $mform->setType($name, PARAM_INT);
            $mform->setDefault($name, $defalut);
            if (isset($design->activityelements[$element])) {
                $mform->setDefault($name, $design->activityelements[$element]);
