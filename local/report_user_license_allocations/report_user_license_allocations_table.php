    ...
    function query_db($pagesize, $useinitialsbar=true) {
        ...
        if (!$this->is_downloading()) {
            if ($this->countsql === NULL) {
                $this->countsql = "SELECT DISTINCT " . $DB-sql_concat("'u.id'", $DB->sql_concat("'-'", $DB->sql_concat("'urla.licenseid'", $DB->sql_concat("'-'", "'urla.courseid"')))) . " AS cindex FROM " . $this->sql->from.' WHERE '.$this->sql->where;
