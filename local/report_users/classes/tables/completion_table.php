...
class completion_table extends table_sql {
    ...
    public function col_finalscore($row) {
        ...
        if (!$DB->record_exists('iomad_courses', ['courseid' => $row->courseid]) ||
            $DB->record_exists('iomad_courses', ['courseid' => $row->courseid, 'hasgrade' => 1])) {
            if ($this->is_downloading() || empty($USER->editing)) {
    ...
    public function col_status($row) {
        ...
        $tooltip .= format_string(get_string('lastmodified') .
                    " - " .
                    userdate($row->modifiedtime, $CFG->iomad_date_format));
