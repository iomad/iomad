...
class oidc_sync {
    ...
    private static function process_users($companyid, $users, $useroption, $unsuspendonsync) {
        ...
        foreach ($users as $aduser) {
            ...
            if (!$founduser = $DB->get_record('user', (array) $userrec)) {
                ...
                // Set the user's language to the company's default language if available.
                // This ensures new users inherit the tenant's language preference instead of defaulting to English.
                if (!empty($company->lang)) {
                    $userrec->lang = $company->lang;
                }
                ...
                // Handle autoenrollment for newly created users
                self::handle_autoenrollment($userrec, $company, $aduser);				
                ...
                // Update user's language to company default if user doesn't have a language set.
                if (!empty($company->lang) && empty($founduser->lang)) {
                    $userrec->lang = $company->lang;
                    // Update the user record in the database.
                    $DB->set_field('user', 'lang', $company->lang, ['id' => $founduser->id]);
                }
     ...
     * @param array $aduser The user data from OIDC
     */
    private static function handle_autoenrollment($user, $company, $aduser) {
        global $CFG;

        try {
            if ($CFG->debug > DEBUG_NONE) {
                mtrace("Processing autoenrollment for user {$user->username} in company {$company->get('name')}");
            }

            // Use the existing company autoenrol method which handles everything:														
            // - Gets courses from company_course_autoenrol table where autoenrol = 1
            // - Handles licensed courses with auto_allocate_license()
            // - Handles non-licensed courses with company_user::enrol()
            // - Triggers proper events for tracking
            $company->autoenrol($user);

            if ($CFG->debug > DEBUG_NONE) {
                mtrace("Autoenrollment completed for user {$user->username}");
            }

        } catch (Exception $e) {
            // Log the error but don't stop the sync process
            mtrace("Error during autoenrollment for user {$user->username}: " . $e->getMessage());
        }
    }	
