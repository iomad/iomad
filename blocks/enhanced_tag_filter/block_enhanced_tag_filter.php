<?php 
class block_enhanced_tag_filter extends block_base {
    public function init() {
        $this->title = get_string('plugintext', 'block_enhanced_tag_filter');
    }
	...
	$resetfilterlabel = get_string('resetfilter', 'block_enhanced_tag_filter');
        ...
        $sql = "SELECT t.id, t.name, t.rawname, t.description, tc.correlatedtags, COUNT(c.id) AS course_count
                ...
                LEFT JOIN {company_course} ac ON ac.courseid = c.id -- join with company assigments
                WHERE (t.isstandard = :isstandard OR t.name LIKE '[%]') -- Include tags in brackets []
                AND ctx.id IS NOT NULL
                AND ac.companyid = :companyid -- restrict to assigend courses
