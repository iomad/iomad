...
class block_iomad_company_admin_external extends external_api {
    ...
    public static function create_companies($companies) {
        ...
        foreach ($params['companies'] as $company) {
            ...
            if(true){
              // Set company manager email settings
              $companydetails->managernotify = 3; // [reminder:1, completion:2, both:3]
              $companydetails->managerdigestday = 6; // [sun:1, mon:2, tue:3, wed:4, thu:5, fri:6, sat:7]
              $companydetails->emailprofileid = 9; // id from page Company Additional Profile Fields
            }
   ...
   /**
    ...
    /**
     * block_iomad_company_admin_find_license
     *
     * Return description of method parameters
     * @return external_function_parameters
     */
    public static function find_license_parameters() {
        return new external_function_parameters(
            array(
              new external_single_structure(
                  array(
                      'courseid' => new external_value(PARAM_INT, 'ID of the course'),
                      'companyid' => new external_value(PARAM_INT, 'ID of the company'),
                  )
              )
            )
        );        
     ...
     *
     * Implement create_company
     * @param array $params  Array with 2 keys [courseid => 1, companyid => 1]
     * @return array $record  License data
     */
    public static function find_license($params) {
        global $CFG, $DB;
        
        // Validate parameters
        $params = self::validate_parameters(self::find_license_parameters(), array($params));
        ...
        $query = ("
          SELECT 
            ...
            cl.parentid
          FROM 
            mdl_companylicense cl,
            mdl_companylicense_courses cc 
          WHERE 
            cc.licenseid = cl.id
          AND cl.companyid = ? 
          AND cc.courseid = ?
        ...
        $params = $params[0]; // was wrapped in array by validate_parameters()
        
        $record = $DB->get_record_sql($query, [$params['companyid'], $params['courseid']]);

        if($record){
          $licenseinfo = array($record);
        } else{
          $licenseinfo = array();
        }
     ...
     * @return external_description
     */
    public static function find_license_returns() {
        return new external_multiple_structure(
            new external_single_structure(
                array(
                    'id' => new external_value(PARAM_INT, 'License ID'),
