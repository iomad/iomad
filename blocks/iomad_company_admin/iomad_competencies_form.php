...
$perpage      = optional_param('perpage', get_config('local_iomad', 'max_list_competencies'), PARAM_INT);        // How many per page.
    ...
    if (!$frameworkdetails = $DB->get_record('iomad_frameworks', ['frameworkid' => $frameworkid])) {
        ...
        if ('shared' == $update) {
            ...
            if ($previousshared == 0 && $shared != 0) { // Turning sharing on.
                // Set the shared group mode options on.
                $DB->set_field('framework', 'groupmode', 1, ['id' => $frameworkid]);
                $DB->set_field('framework', 'groupmodeforce', 1, ['id' => $frameworkid]);

                // Deal with any current enrolments.
                if ($companyframework = $DB->get_record('company_framework', ['frameworkid' => $frameworkid])) {
                    if ($shared == 2) {
                ...
                $frameworkinfo = $DB->get_record('framework', ['id' => $frameworkid]);
                // Set the shared group mode options on.
                $DB->set_field('framework', 'groupmode', 0, ['id' => $frameworkid]);
                $DB->set_field('framework', 'groupmodeforce', 0, ['id' => $frameworkid]);
...
foreach ($frameworks as $framework) {
    ...
    if ($tablecompany = $DB->get_records_sql("SELECT c.shortname FROM {company} c
                                              JOIN {company_comp_framework} ccf ON (c.id = ccf.companyid)
                                              WHERE
    ...
    $frameworklink = new moodle_url('/framework/view.php', ['id' => $framework->id]);
    $table->data[] = [$companyname,
                      html_writer::tag('a', $framework->fullname, ['href' => $frameworklink]),
                      $sharedselectoutput];
}
