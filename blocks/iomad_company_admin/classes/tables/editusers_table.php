...
class editusers_table extends table_sql {
    ...
    public function col_managertype($row) {
        ...
        if (empty($USER->editing) || $selectedcompanyid != $company->id) {
            ...
            if (empty($CFG->iomad_autoenrol_managers)) {
                ...
                $canassigneducators = iomad::has_capability('block/iomad_company_admin:assign_educator', \context_system::instance());
                ...
                if (iomad::has_capability('block/iomad_company_admin:assign_company_reporter', \context_system::instance()) &&
                    ($iseducator || $canassigneducators)) {
                    if (!$canassigneducators) {
            ...
            // Output text if the department doesn't match the currently selected department
    ...
    function __construct($uniqueid) {
        ...
        if (!$CFG->iomad_autoenrol_managers) {
            ...
            if (iomad::has_capability('block/iomad_company_admin:assign_company_reporter', $context)) {
                $this->usertypeselect[41] = get_string('educator', 'block_iomad_company_admin') . ' + ' . get_string('companyreporter', 'block_iomad_company_admin');
            }
        }
    }
}
