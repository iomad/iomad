...
class user_edit_form extends company_moodleform {
    ...
    public function definition() {
        ...
        if (!empty($CFG->emailchangeconfirmation) and !empty($user->preference_newemail)) {
            ...
            if(iomad_helper::has_role('companydepartmentmanager', $USER->id, $this->context)){
              $cdm_email = $DB->get_field('user', 'email', array('id' => $USER->id));
              $mform->setDefault('email', $cdm_email);
            }        
        }
        ...
        $mform->setExpanded('category_id', true); //GH opens the options
...
/*  GH display company not wanted from Behrendt: */
...
/* GH do not show this for Behrendt:  ...
...
GH*/ 
        ...
        if (iomad::has_capability('block/iomad_company_admin:allocate_licenses', $systemcontext)) {
            ...
            $mform->setExpanded('licenses', true); //GH opens the options
...
/* GH: direct course assignment not wanted 2018-12-12
...
*/
    ...
    public function validation($usernew, $files) {
        ...
        // Validate username.
        if (!empty($CFG->iomad_allow_username) && $DB->record_exists('user', array('username' => $usernew->username))) {
