...
class set_acceptances_status extends external_api {
            ...
            [
                'policies' => new external_multiple_structure(
                    new external_single_structure([
                        ...
                        'status' => new external_value(PARAM_INT, 'The iomadpolicy acceptance status. 0: decline, 1: accept.'),
    ...
    public static function execute(array $policies, int $userid = 0): array {
        ...
        foreach ($params['policies'] as $iomadpolicy) {
            $requestedpolicies[$iomadpolicy['versionid']] = $iomadpolicy['status'];
            if ($USER->id != $user->id) {
                // Notes are only allowed when setting acceptances on behalf of another user.
                $notes[$iomadpolicy['versionid']] = $iomadpolicy['note'] ?? null;
            }
        }
        ...
        // Good to go.
        foreach ($agreepolicies as $iomadpolicyversionid) {
            api::accept_policies($iomadpolicyversionid, $user->id, $notes[$iomadpolicyversionid] ?? null);
        }
        foreach ($declinepolicies as $iomadpolicyversionid) {
            api::decline_policies($iomadpolicyversionid, $user->id, $notes[$iomadpolicyversionid] ?? null);
        }

        $return = [
            'iomadpolicyagreed' => (int) $user->iomadpolicyagreed,  // Final iomadpolicy agreement status for $user.
            'warnings' => [],
        ];
        return $return;
    }
    ...
    public static function execute_returns(): external_single_structure {
        return new external_single_structure([
            'iomadpolicyagreed' => new external_value(PARAM_INT,
