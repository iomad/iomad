...
class emailtemplate {
    ...
    public function __construct(string $templatename, array $options = []) {
        ...
        if (empty($this->attachment) && !empty($this->template->id)) {
            ...
            if ($files = $DB->get_records('files', ['contextid' => $context->id,
     ...
     */
    public static function send_to_all_users_in_department($departmentid, $templatename, $options = []) {
    ...
    private static function getuseroption($userrefobject) {
        return ['user' => $userrefobject->userid];
    }
    ...
    public function queue_for_cron() {
        ...
        if (isset($this->user->id)) {
            ...
            if (!empty($this->template->signature)) {
                $email->body = $this->body() . get_string('signatureseparator', 'new ') . $this->signature();
            } else {
            ...
            if (!empty($this->template->emailto)) {
                ...
                foreach ($tousers as $touser) {
                    if ($touserrec = $DB->get_record('user', ['id' => $touser,
                                                              'deleted' => 0,
                                                              'suspended' => 0])) {
            ...
            if (!empty($this->template->emailcc)) {
                ...
                foreach ($ccusers as $ccuser) {
                    if ($ccuserrec = $DB->get_record('user', ['id' => $ccuser,
                                                              'deleted' => 0,
                                                              'suspended' => 0])) {
    ...
    public static function send_to_user($email) {
        ...
        if ($user = self::get_user($email->userid)) {
            ...
            if (!empty($supportuser->customheaders['template'])) {
                ...
                $emailtemplate = new self($email->templatename, ['course' => $email->courseid,
                                                                 'user' => $user,
                                                                 'company' => $company]);
            ...
            if (!empty($template->emailto)) {
                ...
                foreach ($touserids as $touserid) {
                    if ($touser = $DB->get_record('user', ['id' => $touserid,
                                                           'deleted' => 0,
                                                           'suspended' => 0])) {
            ...
            if (!empty($template->emailcc)) {
                ...
                foreach ($ccuserids as $ccuserid) {
                    if ($ccuser = $DB->get_record('user', ['id' => $ccuserid,
                                                           'deleted' => 0,
                                                           'suspended' => 0])) {
                ...
                if (empty($template->disabledmanager)) {
                    ...
                    if ($managers = $company->get_my_managers($email->userid, 1)) {
                        foreach ($managers as $manager) {
                            if ($managerrec = $DB->get_record('user', ['deleted' => 0,
                                                                       'suspended' => 0,
                                                                       'id' => $manager->userid])) {
     ...
     *
     **/
    private static function email_direct($emailaddress,
                                         $supportuser,
                                         $subject,
                                         $messagetext,
                                         $companyid = 0,
    ...
    private function get_course($course) {
        ...
        if ($course) {
            ...
            if (is_int($course) || is_string($course)) {
                if (!$course = $DB->get_record('course', ['id' => $course], '*', MUST_EXIST)) {
    ...
    public static function langpack_imported($event) {
        ...
        $importtask = new importlangpack();
        $importtask->set_custom_data_as_string($newlang);

        // Queue the task.
        manager::queue_adhoc_task($importtask);
