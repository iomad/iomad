...
class block_iomad_onlineusers_generator extends testing_block_generator {
    ...
    public function create_logged_in_users() {
        ...
        for ($i = 1; $i <= 9; $i++) {
            ...
            $DB->insert_record('user_lastaccess', ['userid' => $user->id, 'courseid' => $course1->id, 'timeaccess' => time()]);
            $data['user' . $i] = $user;
        }
        ...
        $data['group2'] = $group2;
        $group3 = $generator->create_group(['courseid' => $course1->id]);
        $data['group3'] = $group3;
        ...
        $generator->create_group_member(['groupid' => $group1->id, 'userid' => $data['user3']->id]);
        ...
        $generator->create_group_member(['groupid' => $group2->id, 'userid' => $data['user5']->id]);
        $generator->create_group_member(['groupid' => $group2->id, 'userid' => $data['user6']->id]);
