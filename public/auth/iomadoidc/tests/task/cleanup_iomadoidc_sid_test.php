...
/**
 * Unit tests for the class cleanup_iomadoidc_sid
 *
 ...
 * @group office365
 * @coversDefaultClass \auth_iomadoidc\task\cleanup_iomadoidc_sid
 */
final class cleanup_iomadoidc_sid_test extends advanced_testcase {
    /**
    ...
    public function test_sids_older_than_yesterday_are_deleted(): void {
        ...
        // Create entries in auth_iomadoidc_sid with unique SIDs.
        ...
        $cleanup = new cleanup_iomadoidc_sid();

        $cleanup->execute();

        $records = $DB->get_records('auth_iomadoidc_sid');

        $this->assertCount(2, $records);

        $this->assertTrue($DB->record_exists('auth_iomadoidc_sid', ['id' => $entry3id]));
        $this->assertFalse($DB->record_exists('auth_iomadoidc_sid', ['id' => $entry1id]));
        $this->assertFalse($DB->record_exists('auth_iomadoidc_sid', ['id' => $entry2id]));
        $this->assertTrue($DB->record_exists('auth_iomadoidc_sid', ['id' => $entry4id]));
    }
}
