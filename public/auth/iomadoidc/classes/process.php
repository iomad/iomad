...
class process {
    /**
     * Route for renaming in auth_iomadoidc.
     *
     * @var int
     */
    const ROUTE_AUTH_IOMADOIDC_RENAME = 1;
    ...
    public function get_file_columns(): array {
        if ($this->filecolumns === null) {
            ...
            if (count($columns) != 2) {
                $this->cir->close();
                $this->cir->cleanup();
                throw new moodle_exception('error_invalid_upload_file', 'auth_iomadoidc');
            }
            ...
            foreach ($columns as $key => $unused) {
                ...
                if (in_array($newfield, $this->filecolumns)) {
                    $this->cir->close();
                    $this->cir->cleanup();
                    throw new moodle_exception('duplicate_upload_field', 'auth_iomadoidc', '', $field);
                }
    ...
    protected function process_line(array $line) {
        ...
        if (!$username || !$lcusername || !$newusername || !$lcnewusername) {
            $this->upt->track('status', get_string('update_error_incomplete_line', 'auth_iomadoidc'));
            $this->userserrors++;
        ...
        if (!$user) {
            ...
            $this->upt->track('status', get_string('update_warning_email_match', 'auth_iomadoidc'));
        }

        if ($user && $user->auth == 'iomadoidc') {
            $route = self::ROUTE_AUTH_IOMADOIDC_RENAME;
        } else {
            $route = self::ROUTE_AUTH_OTHER_MATCH;
        }

        if ($newusername !== core_user::clean_field($newusername, 'username')) {
            $this->upt->track('status', get_string('update_error_invalid_new_username', 'auth_iomadoidc'));
            $this->userserrors++;
        ...
        $userupdated = false;
        $authiomadoidctokenupdated = false;
        $localo365objectupdated = false;

        // Step 1: Update the user object, if route is auth_iomadoidc rename.
        if ($route == self::ROUTE_AUTH_IOMADOIDC_RENAME) {
            ...
            try {
                ...
                $this->upt->track('status', get_string('update_error_user_update_failed', 'auth_iomadoidc'));
                $this->userserrors++;
        ...
        if ($route == self::ROUTE_AUTH_IOMADOIDC_RENAME) {
            if ($tokenrecord = $DB->get_record('auth_iomadoidc_token', ['userid' => $user->id])) {
                $tokenrecord->username = $lcnewusername;
                $tokenrecord->useridentifier = $newusername;
                $DB->update_record('auth_iomadoidc_token', $tokenrecord);
                $authiomadoidctokenupdated = true;
            }
        } else {
            $sql = "SELECT *
        ...
        if (auth_iomadoidc_is_local_365_installed()) {
            if ($route == static::ROUTE_AUTH_IOMADOIDC_RENAME) {
        ...
        if ($userupdated) {
            $this->upt->track('status', get_string('update_success_username', 'auth_iomadoidc'));
        }

        if ($authiomadoidctokenupdated) {
            $this->upt->track('status', get_string('update_success_token', 'auth_iomadoidc'));
        }

        if ($localo365objectupdated) {
            $this->upt->track('status', get_string('update_success_o365', 'auth_iomadoidc'));
        }

        if ($userupdated || $authiomadoidctokenupdated || $localo365objectupdated) {
            // At least one of the records has been updated.
            $this->usersupdated++;
        } else {
            $this->upt->track('status', get_string('update_error_nothing_updated', 'auth_iomadoidc'));
            $this->userserrors++;
        }
    }
    ...
    public function get_stats(): array {
        ...
        $lines[] = get_string('update_stats_users_updated', 'auth_iomadoidc', $this->usersupdated);
        $lines[] = get_string('update_stats_users_errors', 'auth_iomadoidc', $this->userserrors);
