...
class preview extends html_table {
    ...
    public function __construct(csv_import_reader $cir, array $filecolumns, int $previewrows) {
        ...
        $this->head[] = get_string('csvline', 'auth_iomadoidc');
    ...
    protected function read_data(): array {
        ...
        while ($linenum <= $this->previewrows && $fields = $this->cir->next()) {
            ...
            if (!isset($rowcols['username']) || !isset($rowcols['new_username'])) {
                $rowcols['status'][] = get_string('update_error_incomplete_line', 'auth_iomadoidc');
                $hasfatalerror = true;
            }
            ...
            if (!$user) {
                ...
                if ($user) {
                    $rowcols['status'][] = get_string('update_warning_email_match', 'auth_iomadoidc');
                } else {
                    $rowcols['status'][] = get_string('update_error_user_not_found', 'auth_iomadoidc');
                }
            } else if ($user->auth != 'iomadoidc') {
                $rowcols['status'][] = get_string('update_error_user_not_iomadoidc', 'auth_iomadoidc');
            }

            $lcnewusername = core_text::strtolower($rowcols['new_username']);
            if ($lcnewusername != core_user::clean_field($lcnewusername, 'username')) {
                $rowcols['status'][] = get_string('update_error_invalid_new_username', 'auth_iomadoidc');
                $hasfatalerror = true;
            }
