 ...
 */
class auth_iomadoidc_admin_setting_endpoint extends \admin_setting_configtext {
    ...
    public function validate($data) {
        ...
        $idptype = (int) optional_param(
            's_auth_iomadoidc_idptype',
            iomad::get_config('auth_iomadoidc', 'idptype'),
            PARAM_INT
        );
        $clientauthmethod = (int) optional_param(
            's_auth_iomadoidc_clientauthmethod',
            iomad::get_config('auth_iomadoidc', 'clientauthmethod'),
            PARAM_INT
        );

        if (!in_array($idptype, [AUTH_IOMADOIDC_IDP_TYPE_MICROSOFT_ENTRA_ID, AUTH_IOMADOIDC_IDP_TYPE_MICROSOFT_IDENTITY_PLATFORM])) {
            // Non-Microsoft IdP types have no endpoint version requirements.
            return true;
        }

        // The endpoint URL version must match the configured IdP type.
        $endpointversion = auth_iomadoidc_determine_endpoint_version($data);
        if ($endpointversion !== $idptype) {
            ...
            return get_string($mismatchkey, 'auth_iomadoidc');
        }

        // Certificate authentication on Identity Platform requires tenant-specific endpoints.
        if (
            $idptype === AUTH_IOMADOIDC_IDP_TYPE_MICROSOFT_IDENTITY_PLATFORM &&
            $clientauthmethod === AUTH_IOMADOIDC_AUTH_METHOD_CERTIFICATE
        ) {
            if (
                ...
                return get_string('error_tenant_specific_endpoint_required', 'auth_iomadoidc');
            }
        }
