...
use auth_iomadoidc\form\change_binding_username_claim_tool_form1;
use auth_iomadoidc\form\change_binding_username_claim_tool_form2;
use auth_iomadoidc\preview;
use auth_iomadoidc\process;
...
$url = new url('/auth/iomadoidc/change_binding_username_claim_tool.php');
$PAGE->set_url($url);
$PAGE->set_context(system::instance());
$PAGE->set_pagelayout('admin');
$PAGE->set_heading(get_string('settings_page_change_binding_username_claim_tool', 'auth_iomadoidc'));
$PAGE->set_title(get_string('settings_page_change_binding_username_claim_tool', 'auth_iomadoidc'));

admin_externalpage_setup('auth_iomadoidc_change_binding_username_claim_tool');
...
if (empty($iid)) {
    ...
    if ($formdata = $form1->get_data()) {
        ...
        echo $OUTPUT->heading(get_string('change_binding_username_claim_tool', 'auth_iomadoidc'));
        $bindingusernameclaimurl = new url('/admin/settings.php', ['section' => 'auth_iomadoidc_binding_username_claim']);
...
if ($mform2->is_cancelled()) {
    ...
    // Print the header.
    echo $OUTPUT->header();
    echo $OUTPUT->heading(get_string('change_binding_username_claim_tool_result', 'auth_iomadoidc'));
