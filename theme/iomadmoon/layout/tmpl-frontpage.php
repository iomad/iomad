...
// Hidden sidebar.
if (theme_iomadmoon_get_setting('turnoffsidebarfp') == '1' || !isloggedin()) {
    $hiddensidebar = true;
    $navdraweropen = false;
...
$total = 25;
for ($i = 0; $i <= $total; $i++) {
    ${"block" . $i} = theme_iomadmoon_get_setting("block" . $i);
}

$array = array();

for ($i = 0; $i <= $total; $i++) {
    if (theme_iomadmoon_get_setting("displayblock" . $i) == '1') {
        $array[$i] = ${"block" . $i};
    }
...
echo $OUTPUT->render_from_template('theme_iomadmoon/tmpl-frontpage', $templatecontext);
