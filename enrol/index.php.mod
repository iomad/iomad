...
//LZ Check if course is visible - IOMAD specific check
//LZ Modified to be less restrictive for authenticated users attempting self-enrollment
if (class_exists('iomad') && method_exists('iomad', 'iomad_check_course')) {
    if (!iomad::iomad_check_course($id)) {
        // Only block if user is not authenticated or course doesn't exist
        $course_exists = $DB->get_record('course', array('id' => $id), 'id', IGNORE_MISSING);
        if (!$course_exists) {
            print_error('invalidcourseid', 'error');
        }
        // For authenticated users, allow access for potential self-enrollment
    }
}
...
//LZ Check if user has access to the category where the course is located.
//LZ IOMAD FIX: Modified to allow authenticated users to access courses for self-enrollment
if (!is_enrolled($context, $USER, '', true)) {
    //LZ For authenticated users, check if they can view the course info
    //LZ If the method doesn't exist or returns false, allow access for self-enrollment if course is visible
    $can_view_course = true;
    if (method_exists('core_course_category', 'can_view_course_info')) {
        $can_view_course = core_course_category::can_view_course_info($course);
    }
    
    // ChatLLM - Only block access if:
    // 1. User cannot view course info AND
    // 2. Course is not visible AND
    // 3. User doesn't have capability to view hidden courses
    if (!$can_view_course && !$course->visible && !has_capability('moodle/course:viewhiddencourses', $context)) {
