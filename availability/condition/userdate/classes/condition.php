...
/** 
 * User profile data field condition compared to "now", or empty.
 *
 * @package availability_userdate
 * @copyright 2014 The Open University
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class condition extends \core_availability\condition {
    /** @var string Operator: user date field contains date before NOW */
    const OP_BEFORE = 'before';

    /** @var string Operator: user date field contains date after NOW */
    const OP_AFTER = 'after';

    /** @var string Operator: user date field contains date equal NOW 
(todo: rare, because of seconds, need som work)
    */
    ...
    /** @var int Field name (for custom fields) */
    ...
    public function __construct($structure) {
        ...
        // Get operator.
        if (isset($structure->op) && in_array($structure->op, array(self::OP_BEFORE,
                self::OP_AFTER, self::OP_IS_EQUAL_TO, self::OP_IS_EMPTY, self::OP_IS_NOT_EMPTY), true)) {
