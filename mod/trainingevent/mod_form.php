...
// buttons
    ...
    if (!empty($data['instance'])) {
    	...
    	if ($roomclash = $DB->get_records_sql("SELECT * from {trainingevent} where classroomid = ".$data['classroomid']."$mysql AND startdatetime < ".$data['startdatetime']." AND enddatetime > ".$data['startdatetime'])) {
    		$errors['classroomid'] = get_string('chosenclassroomunavailable','trainingevent');
        } else if ($roomclash = $DB->get_records_sql("SELECT * from {trainingevent} where classroomid = ".$data['classroomid']."$mysql AND startdatetime > ".$data['startdatetime']." AND startdatetime < ".$data['enddatetime'])) {
    		$errors['classroomid'] = get_string('chosenclassroomunavailable','trainingevent');
        } else if ($roomclash = $DB->get_records_sql("SELECT * from {trainingevent} where classroomid = ".$data['classroomid']."$mysql AND startdatetime > ".$data['startdatetime']." AND enddatetime < ".$data['enddatetime'])) {
    		$errors['classroomid'] = get_string('chosenclassroomunavailable','trainingevent');
        }
        return $errors;
    }
