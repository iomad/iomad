    ...
    if (!$location = $DB->get_record('classroom',array('id'=>$event->classroomid))) {
                ...
                if (!$DB->get_record('trainingevent_users', array('trainingeventid'=>$id, 'userid'=>$USER->id))) {
                    if (!$DB->insert_record('trainingevent_users', array('trainingeventid'=>$id, 'userid'=>$USER->id))) {
                        ...
                        if ($CFG->perficioemails) {
                            ...
                            EmailTemplate::send('user_signed_up_for_event', array('course'=>$course, 'user'=>$USER, 'classroom'=>$location, 'event' => $event));
                        }
                        add_to_log($event->course, 'trainingevent', 'User attending', 'manageclass.php?id='.$event->id, $event->name .' '.$USER->firstname .' '.$USER->lastname.' (id='.$USER->id.')', $cmidinfo->id, $USER->id);
                    }
                }
            } else if ('no' == $attending) {
                if ($attendingrecord = $DB->get_record('trainingevent_users', array('trainingeventid'=>$id, 'userid'=>$USER->id))) {
                    if (!$DB->delete_records('trainingevent_users', array('id'=>$attendingrecord->id))) {
                        ...
                        if ($CFG->perficioemails) {
                            $course = $DB->get_record('course', array('id'=>$event->course));
                            $location->time = date('jS \of F Y \a\t h:i', $event->startdatetime);
                            EmailTemplate::send('user_removed_from_event', array('course'=>$course, 'user'=>$USER, 'classroom'=>$location, 'event' => $event));
                        }
                        add_to_log($event->course, 'trainingevent', 'User removed', 'manageclass.php?id='.$event->id, $event->name .' '.$USER->firstname .' '.$USER->lastname.' (id='.$USER->id.')', $cmidinfo->id, $USER->id);
                    }
                }
            }
        }
        if (!empty($booking)) {
            if ('yes' == $booking  || 'again' == $booking) {
                if (!$userbooking = $DB->get_record('block_eldms_approve_enroll', array('activityid'=>$id, 'userid'=>$USER->id))) {
                    if (!$DB->insert_record('block_eldms_approve_enroll', array('activityid'=>$id,
                        ...
                        add_to_log($event->course, 'trainingevent', 'User seeking approved access', 'manageclass.php?id='.$event->id, $event->name .' '.$USER->firstname .' '.$USER->lastname.' (id='.$USER->id.')', $cmidinfo->id, $USER->id);
                        }
                    }
                } else {
                    $userbooking->tm_ok = 0;
               ...
               if ($dereq = (array) $DB->get_record('block_eldms_approve_enroll', array('activityid'=>$id, 'userid'=>$USER->id))) {
                   $DB->delete_records('block_eldms_approve_enroll', $dereq);
                        add_to_log($event->course, 'trainingevent', 'User removed approved access', 'manageclass.php?id='.$event->id, $event->name .' '.$USER->firstname .' '.$USER->lastname.' (id='.$USER->id.')', $cmidinfo->id, $USER->id);
               }
           }
        }
        if (!empty($chosen)) {
            ...
            if (!$chosenevent = $DB->get_record('trainingevent', array('id'=>$chosen))) {
                ...
                $chosencmidinfo = $DB->get_record_sql("SELECT * FROM {course_modules}
                 ...
                 if ($alreadyattending < $chosenlocation->capacity) {
                    ...
                    if ($chosenevent->approvaltype == 0 || $chosenevent->approvaltype == 4 || $myapprovallevel == "company" || ($chosenevent->approvaltype == 1 && $myapprovallevel == "department")) {
                         // add to the chosen event
                         if (!$DB->get_record('trainingevent_users', array('userid'=>$userid, 'trainingeventid'=>$chosenevent->id))) {
                             $DB->insert_record('trainingevent_users', array('userid'=>$userid, 'trainingeventid'=>$chosenevent->id));
                             $messagestring = get_string('usermovedsuccessfully', 'trainingevent');
                             $location->time = date('jS \of F Y \a\t h:i', $chosenevent->startdatetime);
                             EmailTemplate::send('user_signed_up_for_event', array('course'=>$course, 'user'=>$user, 'classroom'=>$location, 'event' => $chosenevent));
                         }
                         ...
                         if ($event->approvaltype != 0) {
                            $DB->delete_records('block_eldms_approve_enroll', array('userid' => $userid, 'activityid' => $event->id));
                        }
                        $location->time = date('jS \of F Y \a\t h:i', $event->startdatetime);
                        EmailTemplate::send('user_removed_from_event', array('course'=>$course, 'user'=>$user, 'classroom'=>$location, 'event' => $event));
                        add_to_log($event->course, 'trainingevent', 'User moved to another event', 'manageclass.php?id='.$event->id, $event->name .' '.$user->firstname .' '.$user->lastname.' (id='.$user->id.') by '.$USER->firstname .' '.$USER->lastname.' (id='.$USER->id.')', $cmidinfo->id, $USER->id);
                     } else if (($chosenevent->approvaltype == 3 || $chosenevent->approvaltype == 2) && $myapprovallevel == "department") {
                        // More levels of approval are required.
                        if (!$userbooking = $DB->get_record('block_eldms_approve_enroll', array('activityid'=>$chosenevent->id, 'userid'=>$user->id))) {
                            if (!$DB->insert_record('block_eldms_approve_enroll', array('activityid'=>$chosenevent->id,
                                ...
                                add_to_log($chosenevent->course, 'trainingevent', 'Department manager approved', 'manageclass.php?id='.$event->id, $event->name .' User -'.$user->firstname .' '.$user->lastname.' (id='.$user->id.') by '. $USER->firstname .' '.$USER->lastname.' (id='.$USER->id.')', $chosencmidinfo->id, $USER->id);
                                }
                            }
                        } else {
                            $userbooking->tm_ok = 0;
        ...
        if ($action == 'add' && !empty($userid)) {
            ...
            if ($alreadyattending < $chosenlocation->capacity) {
                //  what kind of event is this?
                if ($event->approvaltype == 0 || $event->approvaltype == 4 || $myapprovallevel == "company" || ($event->approvaltype == 1 && $myapprovallevel == "department")) {
                    // add to the chosen event
                    if (!$DB->get_record('trainingevent_users', array('userid'=>$userid, 'trainingeventid'=>$event->id))) { 
                        ...
                        $location->time = date('jS \of F Y \a\t h:i', $event->startdatetime);
                        EmailTemplate::send('user_signed_up_for_event', array('course'=>$course, 'user'=>$user, 'classroom'=>$location, 'event' => $event));
                        add_to_log($event->course, 'trainingevent', 'User added to event', 'manageclass.php?id='.$event->id, $event->name .' '.$USER->firstname .' '.$USER->lastname.' (id='.$USER->id.')', $cmidinfo->id, $USER->id);
                    }
                    ...
                    if (!$userbooking = $DB->get_record('block_eldms_approve_enroll', array('activityid'=>$id, 'userid'=>$user->id))) {
                        if (!$DB->insert_record('block_eldms_approve_enroll', array('activityid'=>$id,
                            ...
                            add_to_log($event->course, 'trainingevent', 'Department manager approved', 'manageclass.php?id='.$event->id, $event->name .' User -'.$user->firstname .' '.$user->lastname.' (id='.$user->id.') by '. $USER->firstname .' '.$USER->lastname.' (id='.$USER->id.')', $cmidinfo->id, $USER->id);
                            }
                        }
                    } else {
                        $userbooking->tm_ok = 0;
        ...
        if (has_capability('mod/trainingevent:invite', $context)) {
            $eventtable .= "<td>".$OUTPUT->single_button("$CFG->wwwroot/mod/trainingevent/manageclass.php?id=$id&publish=1", get_string('publish','trainingevent')). "</td>";
        }
        if (has_capability('mod/trainingevent:viewattendees', $context)) {
            $eventtable .= "<td>".$OUTPUT->single_button("$CFG->wwwroot/mod/trainingevent/manageclass.php?id=$id&view=1", get_string('viewattendees', 'trainingevent'))."</td>";
        }
        if (has_capability('mod/trainingevent:add', $context) && $numattending < $location->capacity) {
            $eventtable .= "<td>".$OUTPUT->single_button(new moodle_url("/mod/trainingevent/searchusers.php", array('eventid'=>$id)), get_string('selectother','trainingevent')). "</td>";
        }
        $eventtable .= "</tr></table>";
        $eventtable .= "<table>";
        ...
        $eventtable .= "<tr><th>" . get_string('postcode','block_eldms_commerce') . "</th><td>" . $location->postcode . "</td></tr>";
        $eventtable .= "<tr><th>" . get_string('country') . "</th><td>" . $location->country . "</td></tr>";
        $dateformat = "d F Y, g:ia";
         
        $eventtable .= "<tr><th>" . get_string('startdatetime','trainingevent') . "</th><td>" . date($dateformat,$event->startdatetime) . "</td></tr>";
        $eventtable .= "<tr><th>" . get_string('enddatetime','trainingevent') . "</th><td>" . date($dateformat,$event->enddatetime) . "</td></tr>";
        $eventtable .= "<tr><th>" . get_string('capacity','trainingevent') . "</th><td>" . $attendancecount .get_string('of', 'trainingevent') . $location->capacity . "</td></tr>";
        $eventtable .= "</table>";
        ...
        if (!$dodownload) {
                    ...
                    if (time() < $event->startdatetime) {
                        if (!trainingevent_event_clashes($event, $USER->id)) {
                            if ($event->approvaltype == 0) { 
                                ...
                                if (!$mybooking = $DB->get_record('block_eldms_approve_enroll', array('activityid'=>$event->id, 'userid'=>$USER->id))) {
                                    echo $OUTPUT->single_button(new moodle_url('/mod/trainingevent/manageclass.php',
            ...
            if (!empty($view) && has_capability('mod/trainingevent:viewattendees', $context)) {
                ...
                foreach ($courseevents as $courseevent) {
                    ...
                    $eventselect[$courseevent->id] = $courseevent->name . ' - ' . $courselocation->name.' '.date($dateformat,$courseevent->startdatetime);
                } 
                ...
                if ($users = $DB->get_records_sql('SELECT userid as id from {trainingevent_users} where trainingeventid='.$event->id.' AND userid in ('.$allowedlist.')')) {
                    foreach ($users as $user) {
                        ...
                        if (has_capability('mod/trainingevent:add', $context)) {
                            $select = new single_select(new moodle_url('/mod/trainingevent/manageclass.php', array('userid'=>$user->id,'id'=>$event->id, 'view'=>1)), 'chosen', $eventselect, $event->id);
                            $select->formid = 'chooseevent'.$user->id;
                            $eventselecthtml = html_writer::tag('div', $OUTPUT->render($select), array('id'=>'perficio_event_selector'));
                            $removebutton = $OUTPUT->single_button(new moodle_url('manageclass.php', array('userid'=>$user->id, 'id'=>$event->id, 'action'=>'delete','view'=>1 )), get_string("remove",'trainingevent'));
                        ...
                        if (has_capability('mod/trainingevent:grade', $context)) {
                            ...
                            $gradehtml = '<form action="manageclass.php" method="get">
                                         ...
                                         <input type="text" name="usergrade" id="id_usergrade" value="'.$usergradeentry->items[0]->grades[$user->id]->str_grade.'" />
                ...
                echo $OUTPUT->single_button($CFG->wwwroot."/mod/trainingevent/manageclass.php?id=".$id."&dodownload=1", get_string("downloadcsv",'local_report_attendance'));
