    ...
    if (!$location = $DB->get_record('classroom',array('id'=>$event->classroomid))) {
        ...
        if (!empty($booking)) {
            if ('yes' == $booking) {
               ...
               if ($dereq = (array) $DB->get_record('block_iomad_approve_access', array('activityid'=>$id, 'userid'=>$USER->id))) {
        ...
        if  (!empty($publish)) {
            ...
            EmailTemplate::send_to_all_users_in_department($departmentid, 'advertise_classroom_based_course', array('course'=>$course, 'classroom'=>$location));
        ...
        $eventtable .= "<tr><th>" . get_string('postcode','block_iomad_commerce') . "</th><td>" . $location->postcode . "</td></tr>";
        ...
        if (!$dodownload) {
                ...
                if ($attending < $location->capacity) {
                    if (time() < $event->startdatetime) {
                        if (empty($event->approvaltype) || $event->approvaltype == 0) { 
                            ...
                            if (!$DB->get_record('block_iomad_approve_access', array('activityid'=>$event->id, 'userid'=>$USER->id))) {
                                echo $OUTPUT->single_button(new moodle_url('/mod/trainingevent/manageclass.php',
            ...
            if (!empty($view) && has_capability('mod/trainingevent:viewattendees', $context)) {
                ...
                if (has_capability('mod/trainingevent:grade', $context)) {
                    $attendancetable->head = array(get_string('fullname'), get_string('email'), get_string('event','trainingevent'), get_string('action', 'trainingevent'), get_string('grade'));
                    $attendancetable->align = array("left","left","center","center","center");
                } else {
                    $attendancetable->head = array(get_string('fullname'), get_string('email'), get_string('event','trainingevent'), get_string('action', 'trainingevent'));
                ...
                if ($users = $DB->get_records_sql('SELECT userid as id from {trainingevent_users} where trainingeventid='.$event->id.' AND userid in ('.$allowedlist.')')) {
                    foreach ($users as $user) {
                        ...
                        if (has_capability('mod/trainingevent:grade', $context)) {
                            ...
                            $attendancetable->data[] = array($fulluserdata->firstname.' '.$fulluserdata->lastname, $fulluserdata->email, $eventselecthtml, $removebutton, $gradehtml);
                        } else {
                            $attendancetable->data[] = array($fulluserdata->firstname.' '.$fulluserdata->lastname, $fulluserdata->email, $eventselecthtml, $removebutton);
                        }
                    }
                }
